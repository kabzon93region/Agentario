...
### Added
- **AbortSignal для суммаризации** — прокинут AbortSignal через весь pipeline: collectStreamingChunks → generateSummary → runAgenticCompaction. Позволяет прервать зависший запрос к модели
- **Кнопка перезапуска чанка** — в UI для каждого чанка суммаризации отображается кнопка "⟳ Повторить", которая abort текущий запрос и автоматически переотправляет тот же чанк (до 3 ретраев)
- **Safety abort для reasoning-only** — если модель генерирует 1000+ reasoning-чанков подряд без текста, запрос автоматически прерывается с ошибкой

### Improved
- **statusCallback в UI** — прогресс чанков суммаризации снова отображается в чате (с кнопкой перезапуска), а не только в логах
- **Метаданные в сообщениях** — AgentarioMessage расширен полем metadata для action-кнопок

## [0.14.97] - 2026-08-05

### Fixed
- **Дублирование сообщений компакции в UI** — канал statusCallback (мелкие диагностические сообщения "Расчёт:", "Units:", "Чанк N/M:") теперь логируется только в extension host, а не отображается в чате. Пользователь видит только ключевые статусы от emitStatusNotice: контекст до, индикатор работы, результат и контекст после компакции
...
### Fixed
- **Context budget категории = 0 при компакции** — `context_stats` handler в `message-translator.ts` создавал фейковый budget с нулевыми категориями (system=0, rules=0, tools=0) и переззаписывал `pendingContextBudget`. Убрана эмиссия фейкового budget из `context_stats` — теперь только логирует. Реальный budget приходит от `estimateContextBudget` / `CONTEXT_BUDGET_NOTICE_KIND`

### Improved
- **Качество суммаризации** — переписан промпт суммаризатора:
  - Устранено противоречие: правило 2 удаляло пути к файлам, а правило 8 требовало их сохранять
  - Смягчено сжатие: вместо "1.5-2x меньше" — "сохраняй всю полезную информацию, убирай только избыточность"
  - Усилены требования к tool results: обязательно указывать путь, содержимое и результат
  - Увеличен минимум слов с 30 до 50
  - Усилен DEFAULT_PROMPT_AFTER с акцентом на сохранение путей и содержимого файлов

## [0.14.95] - 2026-08-05

### Fixed
- **CycleDetector не обнаруживал зацикливание thinking** — `onThinking` вызывался в `content_end` handler, но `event.reasoning` мог быть пустым (контент уже приходил через `content_start` deltas). Исправлено: используется накопленный текст из `getStreamingReasoningText()` перед очисткой буфера. Также добавлен `getStreamingReasoningText()` метод в `MessageTranslatorState`
...
esult (обрезанный до 2,000 chars) и error, а не считает полный JSON.stringify. Исправляет завышение оценки "на суммаризацию" в ~3 раза (например, 13,167 ток. → реальные ~3,155 ток. в чанках)
...
### Fixed
- **Критический фикс: потеря содержимого tool results при компакции контекста** — функции lattenToolResultContent и lattenToolResultContentFull не обрабатывали structured tool outputs (ToolOperationResult[] с полями query, 
esult, error, success), возвращаемые инструментами 
un_commands и 
ead_files. Эти объекты попадали в default: return "" и терялись при компакции, что приводило к ложному выводу суммаризатора "файлы недоступны". Добавлена функция ormatStructuredToolEntry для извлечения содержимого из structured entries
- **Усилен промпт суммаризации** — добавлено правило №11: ошибки tool call (например, ENOENT) не означают отсутствие файла; файл может быть прочитан другим tool call
...
### Fixed
- **Stats для ВСЕХ сообщений в UI** — indApiStatsForMessage теперь ищет stats backward (к предыдущему pi_req_started), а не forward. Исправляет проблему, когда stats отображались только у последнего сообщения, а у промежуточных (tool, command, thinking) — нет. Теперь каждый tool/command/thinking получает stats от своего API-вызова
