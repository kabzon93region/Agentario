...
import { type HookEventPayload, parseHookEventPayload } from "@agentario/shared";
...
export async function appendHookAudit(event: HookEventPayload): Promise<void> {
	...
	const envPath = process.env.agentario_HOOKS_LOG_PATH?.trim() || undefined;
...
export function resolveSandboxDataDir(
	...
	const envDir = process.env.agentario_SANDBOX_DATA_DIR?.trim();
	const baseDir =
...
export function configureSandboxEnvironment(options: {
	...
	process.env.agentario_SANDBOX = "1";
	process.env.agentario_SANDBOX_DATA_DIR = dataDir;
	process.env.agentario_DATA_DIR = dataDir;
	process.env.agentario_DB_DATA_DIR = join(dataDir, "db");
	process.env.agentario_SESSION_DATA_DIR = join(dataDir, "sessions");
	process.env.agentario_TEAM_DATA_DIR = join(dataDir, "teams");
	process.env.agentario_PROVIDER_SETTINGS_PATH = join(
		dataDir,
		"settings",
		"providers.json",
	);
	process.env.agentario_HOOKS_LOG_PATH = join(dataDir, "logs", "hooks.jsonl");
	return dataDir;
}
