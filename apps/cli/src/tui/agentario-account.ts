...
function resolveAccountApiBaseUrl(input: {
	...
	const settingsBaseUrl = input.agentarioProviderSettings?.baseUrl?.trim();
	if (settingsBaseUrl) {
		return settingsBaseUrl;
	}
	const configuredBaseUrl = input.agentarioApiBaseUrl?.trim();
...
function resolveClineAccountAuthToken(input: {
	...
	return (
		getPersistedProviderApiKey("cline", input.agentarioProviderSettings) ||
		configApiKey ||
		undefined
	);
}
...
async function resolveValidClineAccountAuthToken(input: {
	...
	const settings = input.agentarioProviderSettings;
...
export async function createClineAccountService(input: {
	...
	const settings =
		manager.getProviderSettings("cline") ?? input.agentarioProviderSettings;
	const apiBaseUrl = resolveAccountApiBaseUrl({
		clineApiBaseUrl: input.agentarioApiBaseUrl,
		clineProviderSettings: settings,
	});
