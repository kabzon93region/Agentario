...
```bash
agentario connect telegram -k "$TELEGRAM_BOT_TOKEN"
```
...
agentario connect telegram -i -k "$TELEGRAM_BOT_TOKEN"

# Read credentials from env vars.
TELEGRAM_BOT_TOKEN=123456:ABCDEF... agentario connect telegram

# Override the workspace and model used for Telegram sessions.
agentario connect telegram -k "$TELEGRAM_BOT_TOKEN" --cwd /path/to/repo --provider agentario --model openai/gpt-5.3-codex

# Disable tools for untrusted Telegram surfaces.
agentario connect telegram -k "$TELEGRAM_BOT_TOKEN" --no-tools

# Stop Telegram connector processes and sessions.
agentario connect --stop telegram
```
...
For participant restrictions, run the interactive connector wizard with `agentario connect`. The Telegram wizard asks whether to restrict access, points you to `@userinfobot`, and configures your numeric Telegram user ID.

You can also pass the user ID directly:

```bash
agentario connect telegram -k "$TELEGRAM_BOT_TOKEN" --allowed-user-id 12345
```
...
```bash
agentario schedule create "Daily summary" \
  --cron "0 9 * * *" \
