...
interface AgentarioModelPickerProps {
	isPopup?: boolean
	currentMode: Mode
...
function toFeaturedModelCardEntry(
	model: Pick<AgentarioRecommendedModel, "id" | "description" | "tags">,
...
const RECOMMENDED_MODELS_FALLBACK: FeaturedModelCardEntry[] = AGENTARIO_RECOMMENDED_MODELS_FALLBACK.recommended
	.map((model) => toFeaturedModelCardEntry(model, "RECOMMENDED"))
	.filter((model): model is FeaturedModelCardEntry => model !== null)

const FREE_MODELS_FALLBACK: FeaturedModelCardEntry[] = AGENTARIO_RECOMMENDED_MODELS_FALLBACK.free
	.map((model) => toFeaturedModelCardEntry(model, "FREE"))
	.filter((model): model is FeaturedModelCardEntry => model !== null)

const AgentarioModelPicker: React.FC<AgentarioModelPickerProps> = ({ isPopup, currentMode, showProviderRouting, initialTab }) => {
	...
	const [clineRecommendedModels, setAgentarioRecommendedModels] = useState<FeaturedModelCardEntry[]>([])
	...
	const hasSuccessfulAgentarioRecommendedModelsFetchRef = useRef(false)
	const isFetchingAgentarioRecommendedModelsRef = useRef(false)
	const clineRecommendedModelsRetryTimeoutRef = useRef<number | null>(null)

	const refreshAgentarioRecommendedModels = useCallback(async (): Promise<boolean> => {
		try {
			const response = await ModelsServiceClient.makeUnaryRequest(
				"refreshAgentarioRecommendedModelsRpc",
				EmptyRequest.create({}),
				EmptyRequest.toJSON,
				AgentarioRecommendedModelsResponse.fromJSON,
			)
			...
			const free = (response.free ?? [])
				.map((model) => toFeaturedModelCardEntry(model, "FREE"))
				.filter((model): model is FeaturedModelCardEntry => model !== null)
			setAgentarioRecommendedModels(recommended)
	...
	const clearAgentarioRecommendedModelsRetryTimeout = useCallback(() => {
	...
	const fetchAgentarioRecommendedModels = useCallback(async () => {
		if (hasSuccessfulAgentarioRecommendedModelsFetchRef.current || isFetchingAgentarioRecommendedModelsRef.current) {
			return
		}
		isFetchingAgentarioRecommendedModelsRef.current = true
		const succeeded = await refreshAgentarioRecommendedModels()
		isFetchingAgentarioRecommendedModelsRef.current = false

		if (succeeded) {
			hasSuccessfulAgentarioRecommendedModelsFetchRef.current = true
			clearAgentarioRecommendedModelsRetryTimeout()
			return
		}
		...
		if (clineRecommendedModelsRetryTimeoutRef.current === null) {
			clineRecommendedModelsRetryTimeoutRef.current = window.setTimeout(() => {
				clineRecommendedModelsRetryTimeoutRef.current = null
				void fetchAgentarioRecommendedModels()
			}, CLINE_RECOMMENDED_MODELS_RETRY_DELAY_MS)
		}
	}, [clearAgentarioRecommendedModelsRetryTimeout, refreshAgentarioRecommendedModels])
		...
		void handleModeFieldsChange(
			{
				...
				clineModelInfo: {
					plan: "planModeAgentarioModelInfo",
					act: "actModeAgentarioModelInfo",
				},
			},
			{
	...
	useEffect(() => {
		void fetchAgentarioRecommendedModels()
	}, [fetchAgentarioRecommendedModels])
...
export default AgentarioModelPicker
