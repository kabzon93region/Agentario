...
export const LMStudioProvider = ({ currentMode, showModelOptions }: LMStudioProviderProps) => {
	...
	const { config, write, commitSelection, isLoading } = useProviderConfig("lmstudio")

	const [lmStudioModels, setLmStudioModels] = useState<LmStudioApiModel[]>([])
	const sortedLmStudioModels = useMemo(() => sortLmStudioModelsForPicker(lmStudioModels), [lmStudioModels])
	...
	// Keep committed selection contextWindow in sync with LM Studio runtime (TaskHeader bar).
	useEffect(() => {
		if (!currentLMStudioModel || !displayedSelectedModelId) {
			return
		}
		const contextWindow =
			currentLMStudioModel.loaded_context_length ?? currentLMStudioModel.max_context_length
		if (!contextWindow || contextWindow <= 0) {
			return
		}
		const committedInfo =
			currentMode === "plan" ? config?.planSelection?.modelInfo : config?.actSelection?.modelInfo
		if (committedInfo?.contextWindow === contextWindow && selectedModel.modelId === displayedSelectedModelId) {
			return
		}
		void commitModelSelection({
			modelId: displayedSelectedModelId,
			modelInfo: toLmStudioModelInfo(currentLMStudioModel, displayedSelectedModelId),
		}).catch((error) => console.error("Failed to sync LM Studio context window:", error))
	}, [
		commitModelSelection,
		config?.actSelection?.modelInfo,
		config?.planSelection?.modelInfo,
	...
	return (
		<div className="flex flex-col gap-2">
			...
			{sortedLmStudioModels.length > 0 ? (
				<DropdownContainer className="dropdown-container" zIndex={10}>
					<VSCodeDropdown
						...
						{sortedLmStudioModels.map((model) => (
							<VSCodeOption className="w-full" key={model.id} value={model.id}>
								{formatLmStudioAgentModelLabel(model)}
							</VSCodeOption>
						))}
					</VSCodeDropdown>
				</DropdownContainer>
			...
			<div className="text-xs text-description">
				...
				<p className="mt-1">{t("providers.lmstudio.modelListHint")}</p>
			</div>
				</>
			)}
		</div>
