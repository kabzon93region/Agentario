...
function seqOf(message: AgentarioMessage): number {
	return message.seq ?? 0
}

/** Replace the replica's transcript wholesale at a new epoch (new task / history load). */
function resetTo(epoch: number, messages: AgentarioMessage[], stateVersion: number, turnState?: TurnState): ReplicaState {
...
/**
 * Apply one incoming AgentarioMessage (from the partial-message stream OR from within a state
 ...
 */
export function applyMessage(state: ReplicaState, incoming: AgentarioMessage): ReplicaState {
...
export function applyStateSnapshot(
	state: ReplicaState,
	snapshotMessages: AgentarioMessage[],
	snapshotEpoch = 0,
	snapshotVersion = 0,
