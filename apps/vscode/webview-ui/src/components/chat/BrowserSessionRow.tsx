	...
	const pages = useMemo(() => {
		const result: {
			currentState: {
				...
				consoleLogs?: string
				messages: AgentarioMessage[] // messages up to and including the result
			}
			nextAction?: {
				messages: AgentarioMessage[] // messages leading to next result
			}
		}[] = []

		let currentStateMessages: AgentarioMessage[] = []
		let nextActionMessages: AgentarioMessage[] = []
		...
		<BrowserSessionRowContainer style={{ marginBottom: -10 }}>
			<div style={browserSessionRowContainerInnerStyle}>
				...
				<span style={approveTextStyle}>
					{isAutoApproved ? "Agentario is using the browser:" : "Agentario wants to use the browser:"}
				</span>
			</div>
			<div
				style={{
