...
// (`@agentario/core` `fetchAgentarioRecommendedModels`). These tests cover the
...
describe("refreshAgentarioRecommendedModels", () => {
	beforeEach(() => {
		resetAgentarioRecommendedModelsCacheForTests()
	...
	it("delegates to the SDK fetch", async () => {
		...
		const sdkSpy = vi.spyOn(sdkCore, "fetchAgentarioRecommendedModels").mockResolvedValue(sdkResult)

		const result = await refreshAgentarioRecommendedModels()
	...
	it("uses the in-memory cache after a populated upstream result", async () => {
		...
		const firstResult = await refreshAgentarioRecommendedModels()
		const secondResult = await refreshAgentarioRecommendedModels()
	...
	it("does not cache the SDK fallback result", async () => {
		...
		const sdkSpy = vi
			.spyOn(sdkCore, "fetchAgentarioRecommendedModels")
