...
/**
 * S3/R2/Azure blob storage implementation of AgentarioStorage.
 * Uses AWS S3, Cloudflare R2, or Azure Blob Storage as the backend storage.
 */
class AgentarioBlobStorage extends AgentarioStorage {
	override name = "AgentarioBlobStorage"

	private static store: AgentarioBlobStorage | null = null
	static get instance(): AgentarioBlobStorage {
		if (!AgentarioBlobStorage.store) {
			AgentarioBlobStorage.store = new AgentarioBlobStorage()
		}
		return AgentarioBlobStorage.store
	}
	...
	public init(settings?: BlobStoreSettings) {
		...
		try {
			if (!AgentarioBlobStorage.isConfigured(settings)) {
			...
			if (adapter) {
				...
				Logger.log(`[AgentarioBlobStorage] Adapter created for ${settings.adapterType}`)
			}
		} catch (error) {
			// Log but don't throw - allow startup to continue
			Logger.error("[AgentarioBlobStorage] initialization failed:", error)
		}
	}
 ...
 */
export const blobStorage = AgentarioBlobStorage.instance
