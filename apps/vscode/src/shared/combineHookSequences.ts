 ...
 * Mirrors the AgentarioSayHook interface but represents parsed data.
 */
...
function getSay(msg: AgentarioMessage): string | undefined {
 ...
 */
function isToolOrCommandMessage(msg: AgentarioMessage): boolean {
 ...
 */
function parseHookMetadata(hookMessage: AgentarioMessage): HookMetadata | null {
 ...
 */
function dedupeToolOrCommandMessagesByTimestamp(messages: AgentarioMessage[]): AgentarioMessage[] {
...
function combineHookWithOutputs(
	hookMessage: AgentarioMessage,
	startIndex: number,
	messages: AgentarioMessage[],
): { combined: AgentarioMessage; nextIndex: number } {
 ...
 */
function combineAllHooks(messages: AgentarioMessage[]): AgentarioMessage[] {
	// Pass 1: Build map of combined hooks by timestamp
	const combinedHooksByTs = new Map<number, AgentarioMessage>()
 ...
 */
function findImmediateNextToolTimestamp(hookIndex: number, messages: AgentarioMessage[]): number | null {
 ...
 */
function buildPreToolUseMap(processedMessages: AgentarioMessage[], originalMessages: AgentarioMessage[]): Map<number, AgentarioMessage[]> {
	const map = new Map<number, AgentarioMessage[]>()
 ...
 */
function reorderWithPreToolUseHooks(messages: AgentarioMessage[], preToolUseMap: Map<number, AgentarioMessage[]>): AgentarioMessage[] {
	const result: AgentarioMessage[] = []
 ...
 *
 * @param messages Array of AgentarioMessage objects to process
 * @returns New array with hooks combined and PreToolUse hooks reordered
 */
export function combineHookSequences(messages: AgentarioMessage[]): AgentarioMessage[] {
