...
export async function compareCheckpointToWorkspace(): Promise<CheckpointWorkspaceCompareResult> {
	throw new Error("compareCheckpointToWorkspace is not implemented in the Vitest @agentario/core stub")
}
...
export function createAgentarioTelemetryServiceConfig(config: Record<string, unknown> = {}) {
	return {
		enabled: false,
		metadata: {
...
export function getProviderAuthStorageId(providerId: string): string | undefined {
	...
	if (normalized === "cline" || normalized === "agentario-pass") {
		return "cline"
	}
...
export async function listLocalProviders(
	...
	options: { isAgentarioPassEnabled?: boolean } = {},
	...
	const providers = Object.entries(MODEL_COLLECTIONS_BY_PROVIDER_ID)
		...
		.filter((provider) => options.isAgentarioPassEnabled === true || provider.id !== "agentario-pass")
...
export interface AgentarioRecommendedModel {
	id: string
	name: string
	description: string
	tags: string[]
...
export const FALLBACK_CLINE_RECOMMENDED_MODELS: AgentarioRecommendedModelsData = {
	recommended: [
		{
...
export async function fetchAgentarioRecommendedModels(_options?: {
	baseUrl?: string
	fetchImpl?: typeof fetch
}): Promise<AgentarioRecommendedModelsData> {
