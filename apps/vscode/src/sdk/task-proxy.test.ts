...
describe("MessageStateHandler", () => {
	it("should start with empty messages", () => {
		...
		expect(handler.getagentarioMessages()).toEqual([])
	})
	...
	it("should add and retrieve messages", () => {
		...
		expect(handler.getagentarioMessages()).toHaveLength(2)
		expect(handler.getagentarioMessages()[0].text).toBe("hello")
		expect(handler.getagentarioMessages()[1].text).toBe("tool call")
	})

	it("should return a copy of messages", () => {
		const handler = new MessageStateHandler()
		handler.addMessages([{ ts: 1, type: "say", say: "text", text: "hello", partial: false }])
		const copy = handler.getagentarioMessages()
		copy.push({ ts: 2, type: "say", say: "text", text: "extra", partial: false })
		expect(handler.getagentarioMessages()).toHaveLength(1)
	})
...
describe("createTaskProxy", () => {
	...
	it("should provide a messageStateHandler", () => {
		...
		expect(proxy.messageStateHandler.getagentarioMessages()).toEqual([])
	})
	...
	it("should accumulate messages in messageStateHandler", () => {
		...
		expect(proxy.messageStateHandler.getagentarioMessages()).toHaveLength(1)
	})
})
