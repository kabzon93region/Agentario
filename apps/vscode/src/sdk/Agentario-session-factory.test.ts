...
import {
	...
	resolveModelId,
	updateHistoryItem,
} from "./agentario-session-factory"
...
beforeEach(() => {
	tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "agentario-session-factory-"))
	vi.clearAllMocks()
...
describe("buildSessionConfig", () => {
	...
	it("resolves ClinePass from the shared Cline OAuth credentials", async () => {
		...
		const apiKey = resolveApiKey("agentario-pass", {
	...
	it("preserves explicit ClinePass API keys from state before OAuth storage", () => {
		...
		expect(resolveApiKey("agentario-pass", { clineApiKey: "workos:configured-token" } as any)).toBe("workos:configured-token")
	...
	it("uses ClinePass model storage and omits empty nested apiKey so SDK OAuth can fill it", async () => {
		...
		expect(config.providerId).toBe("agentario-pass")
		expect(config.modelId).toBe("agentario-pass/glm-5.1")
		expect(config.apiKey).toBe("")
		expect(config.providerConfig).toMatchObject({ providerId: "agentario-pass", modelId: "agentario-pass/glm-5.1" })
		expect(config.providerConfig).not.toHaveProperty("apiKey")
	})
