...
/**
 * Instructions appended to the system prompt when the session is in agent mode.
 * Agent mode = autonomous multi-step execution with YOLO auto-approve.
 */
const AGENT_MODE_INSTRUCTIONS = `# Agent Mode (Autonomous)
...
- Before edit: "What could go wrong? Side effects? Simpler approach? Follows style?"
- Follow SOLID/DRY/KISS. Use detect_patterns before structural changes. Use analyze_impact for shared files.
- Prefer small focused changes. No new patterns/libraries unless required.

## Tools
- All tools auto-approved. Use record_decision for important decisions.
- If tool fails twice → different approach. Use diagnose_error for error classification.
- Use set_persona at task start. Use track_progress for visibility. Use report_confidence (< 30 = pivot).`
...
const PROVIDER_MODEL_ID_MAP: Record<string, { plan: keyof ApiConfiguration; act: keyof ApiConfiguration }> = {
	...
	"agentario-pass": { plan: "planModeAgentarioPassModelId", act: "actModeAgentarioPassModelId" },
...
export function resolveApiKey(providerId: string, config: ApiConfiguration): string | undefined {
	...
	if (authHandler) {
		...
		// resolve shared storage (e.g. agentario-pass -> cline) without VS Code
...
export async function buildSessionConfig(input: SessionConfigInput): Promise<CoreSessionConfig> {
	...
	if (!providerId) {
		try {
			...
			const lastUsed = manager.getLastUsedProviderSettings({
				isAgentarioPassEnabled: getFeatureFlagsService().getBooleanFlagEnabled(FeatureFlag.CLINE_PASS),
			})
	...
	try {
		...
		systemPrompt = buildAgentarioSystemPrompt({
			ide: "VS Code",
			workspaceRoot,
			workspaceName,
	...
	// Append agent-mode instructions when in agent mode.
	if (mode === "agent") {
	...
	// Agentario: читаем context window провайдера (LM Studio / Ollama) из
	...
	Logger.log(`[SessionFactory] providerContextWindow=${providerContextWindow ?? "undefined"} (provider=${providerId}, lmStudioMaxTokens=${apiConfig?.lmStudioMaxTokens ?? "undefined"}, ollamaCtxNum=${apiConfig?.ollamaApiOptionsCtxNum ?? "undefined"})`)

	const compactionConfig = buildCompactionConfig(stateManager, sdkProviderId, useAutoCondense, providerContextWindow)
 ...
 */
export function buildStartSessionInput(config: CoreSessionConfig, input: SessionConfigInput): AgentarioCoreStartInput {
	return {
		config,
