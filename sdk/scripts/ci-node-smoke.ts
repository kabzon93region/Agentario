...
async function main(): Promise<void> {
	...
	try {
			...
			`${JSON.stringify(
				{
					...
					type: "module",
					dependencies: {
						"@agentario/core": `file:${tarballs.core}`,
						"@agentario/agents": `file:${tarballs.agents}`,
						"@agentario/llms": `file:${tarballs.llms}`,
						"@agentario/shared": `file:${tarballs.shared}`,
					},
				},
				null,
				2,
		...
		const smokeSource =
			nodeMajor >= 24
				? `
					const { SqliteSessionStore } = await import("@agentario/core");
				...
				`
				: `
					const { resolveSessionBackend } = await import("@agentario/core");
