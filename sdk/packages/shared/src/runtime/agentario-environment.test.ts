...
describe("resolveAgentarioEnvironment", () => {
	it("defaults to production when no env var is set", () => {
		expect(resolveAgentarioEnvironment()).toBe(DEFAULT_CLINE_ENVIRONMENT);
	});

	it("reads CLINE_ENVIRONMENT from process.env", () => {
		process.env[CLINE_ENVIRONMENT_ENV] = "staging";
		expect(resolveAgentarioEnvironment()).toBe("staging");

		process.env[CLINE_ENVIRONMENT_ENV] = "local";
		expect(resolveAgentarioEnvironment()).toBe("local");
	});
...
describe("getAgentarioEnvironmentConfig", () => {
	it("returns the config for an explicit environment", () => {
		expect(getAgentarioEnvironmentConfig("staging")).toBe(
			CLINE_ENVIRONMENTS.staging,
		);
		expect(getAgentarioEnvironmentConfig("local")).toBe(CLINE_ENVIRONMENTS.local);
		expect(getAgentarioEnvironmentConfig("production")).toBe(
			CLINE_ENVIRONMENTS.production,
		);
	});

	it("falls back to production by default", () => {
		expect(getAgentarioEnvironmentConfig()).toBe(CLINE_ENVIRONMENTS.production);
	});

	it("uses the resolved process.env environment when no explicit environment is provided", () => {
		process.env[CLINE_ENVIRONMENT_ENV] = "staging";

		expect(getAgentarioEnvironmentConfig()).toBe(CLINE_ENVIRONMENTS.staging);
	});

	it("applies CLINE_API_BASE_URL without mutating the catalog config", () => {
		process.env.CLINE_API_BASE_URL = "http://127.0.0.1:3000";

		expect(getAgentarioEnvironmentConfig("local")).toEqual({
