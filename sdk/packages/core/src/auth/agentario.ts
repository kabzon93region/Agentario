...
export async function loginAgentarioCloudOAuth(
	options: ClineOAuthProviderOptions & {
	...
	try {
		...
		if (useWorkOSDeviceAuth) {
			const clientId = getAgentarioEnvironmentConfig().workOsClientId;
...
export async function startAgentarioCloudDeviceAuth(options?: {
	requestTimeoutMs?: number;
}): Promise<{
	deviceCode: string;
	userCode: string;
	...
	return await requestWorkOSDeviceAuthorization(
		getAgentarioEnvironmentConfig().workOsClientId,
		options,
	);
}
...
export async function completeClineDeviceAuth(options: {
	...
	try {
		const workosTokens = await pollWorkOSTokens({
			clientId: getAgentarioEnvironmentConfig().workOsClientId,
...
export async function refreshAgentarioCloudToken(
...
export async function getValidClineCredentials(
	...
	try {
		return await refreshAgentarioCloudToken(currentCredentials, providerOptions);
	} catch (error) {
