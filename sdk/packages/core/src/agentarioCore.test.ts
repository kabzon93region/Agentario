...
import type { AgentarioCoreStartInput } from "./agentario-core/types";
import type {
	StartSessionInput,
	StartSessionResult,
...
import { AgentarioCore } from "./AgentarioCore";
import { NoOpFeatureFlagsProvider } from "./services/feature-flags";

function createStartInput(): AgentarioCoreStartInput {
	return {
		config: {
...
describe("AgentarioCore", () => {
	...
	it("compares a checkpoint to the current workspace through the public SDK API", async () => {
		...
		try {
			...
			core = await AgentarioCore.create();
	...
	it("applies start-session bootstraps before delegating to the host", async () => {
		...
		const applyToStartSessionInput = vi.fn(
			async (input: AgentarioCoreStartInput) => ({
				...input,
				config: {
					...input.config,
		...
		const core = await AgentarioCore.create({
			prepare: async () => ({
	...
	it("prefers the per-session telemetry service over the AgentarioCore one", async () => {
		const host = {
			runtimeAddress: undefined,
	...
	it("can list sessions without hydrating message history", async () => {
		...
		// AgentarioCore/listSessionHistory; the host list contract only receives the
		// numeric scan limit.
	...
	it("exposes event automation through AgentarioCore instead of CronService", async () => {
		const root = mkdtempSync(join(tmpdir(), "agentario-core-automation-"));
