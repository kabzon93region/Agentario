...
export function normalizeAutomationOptions(
	options: AgentarioCoreAutomationOptions | boolean | undefined,
): AgentarioCoreAutomationOptions | undefined {
...
export function normalizeAutomationCronScope(
	scope: AgentarioCoreAutomationOptions["cronScope"],
): "global" | "workspace" | undefined {
	if (scope === "user") return "global";
	return scope;
}

export class AgentarioCoreAutomationController implements AgentarioCoreAutomationApi {
...
export function createAgentarioCoreAutomationRuntimeHandlers(
	...
	return {
		...
		async sendSession(sessionId, request) {
			...
			if (!result) {
				throw new Error("AgentarioCore automation runtime returned no result");
			}
			return { result: toChatTurnResult(result) };
		},
		async abortSession(sessionId) {
			await host.abort(sessionId, new Error("AgentarioCore automation abort"));
			return { applied: true };
		},
...
export interface ClineCoreAutomationExtensionContextInput {
	...
	automation: AgentarioCoreAutomationApi;
...
export function createAgentarioCoreAutomationExtensionContext(
