...
import { PLUGIN_FILE_EXTENSIONS } from "@agentario/shared";
...
const HOST_PROVIDED_SDK_SPECIFIERS = [
	...
	"@agentario/core",
	"@agentario/core/hub",
	"@agentario/core/hub/daemon-entry",
	"@agentario/core/telemetry",
	"@agentario/llms",
	"@agentario/llms/browser",
	"@agentario/shared",
	"@agentario/shared/automation",
	"@agentario/shared/browser",
	"@agentario/shared/storage",
	"@agentario/shared/db",
	"@agentario/shared/types",
];
...
function collectWorkspaceAliases(root: string): Record<string, string> {
	...
	const candidates: Record<string, string> = {
		"@agentario/sdk": resolve(root, "packages/sdk/src/index.ts"),
		"@agentario/agents": resolve(root, "packages/agents/src/index.ts"),
		"@agentario/core": resolve(root, "packages/core/src/index.ts"),
		"@agentario/llms": resolve(root, "packages/llms/src/index.ts"),
		"@agentario/shared": resolve(root, "packages/shared/src/index.ts"),
		"@agentario/shared/storage": resolve(
			root,
			"packages/shared/src/storage/index.ts",
		),
		"@agentario/shared/db": resolve(root, "packages/shared/src/db/index.ts"),
	};
...
function isClineSdkSpecifier(specifier: string): boolean {
	return getPackageName(specifier).startsWith("@agentario/");
}
...
export async function importPluginModule(
	...
	const jiti = createJiti(pluginPath, {
		...
		// every nested import in the plugin, sees `import "@agentario/core"` with no
