...
// agentario-pass models have only the model name (and not the lab),
// so we need to look-up using glm-5.1 instead of agentario-pass/glm-5.1
...
export function normalizeAgentarioRecommendedProviderModels(
...
export async function fetchClineRecommendedModelsPayload(
	...
	const url = `${getAgentarioEnvironmentConfig().apiBaseUrl}/api/v1/ai/cline/recommended-models`;
...
export async function fetchClineRecommendedProviderModels(
	...
	return normalizeAgentarioRecommendedProviderModels(payload, openRouterModels);
}
