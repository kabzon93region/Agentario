...
2. Add them to a [Cline Rules file](/customization/agentario-rules), such as `.clinerules/memory-bank.md`
...
- Use [Cline Rules](/customization/agentario-rules) to store the Memory Bank instructions per project
...
Either works. Custom instructions apply globally across all projects. A [Cline Rules file](/customization/agentario-rules) is project-specific and stored in your repo, which makes it easy to share with collaborators. You can also use [conditional rules](/customization/agentario-rules#conditional-rules) to activate Memory Bank instructions only when working with `memory-bank/` files.
