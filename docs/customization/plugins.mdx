...
Dependencies under the `@agentario/` scope are provided by the host runtime. The installer automatically strips these from the plugin's dependency list before running `npm install`, so declare any `@agentario/*` package your plugin imports as an optional peer dependency. The host currently provides `@agentario/sdk`, `@agentario/core`, `@agentario/agents`, `@agentario/llms`, and `@agentario/shared`.
