...
description: "API reference for the LLM provider gateway in @agentario/llms."
---

`DefaultGateway` creates provider-backed `AgentModel` instances and exposes provider/model catalog helpers.

```typescript
import { DefaultGateway, createGateway } from "@agentario/llms"
```
