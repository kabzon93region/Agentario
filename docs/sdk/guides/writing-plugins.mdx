...
```typescript
// github-plugin.ts
import { type AgentPlugin } from "@agentario/sdk"
import { createTool } from "@agentario/sdk"
...
import { type AgentPlugin, createTool } from "@agentario/sdk"
...
```typescript
import { ClineCore } from "@agentario/sdk"
...
Single-file plugins can only import Node builtins and `@agentario/*`. As soon as you need an npm dependency (`zod`, an HTTP client, etc.) you must ship as a package: a directory with a `package.json` that declares a `cline` field for entry points and your runtime `dependencies`. Dependencies under the `@agentario/` scope are provided by the host runtime -- the installer strips these and runs `npm install` for the rest, so declare any `@agentario/*` package you import as an optional peer dependency:
