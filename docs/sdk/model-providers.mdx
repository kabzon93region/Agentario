...
The `@agentario/llms` package provides provider registries, model catalogs, handlers, and a gateway. `@agentario/agents` and `@agentario/core` use this layer internally.
...
```typescript
import { DefaultGateway } from "@agentario/llms"
